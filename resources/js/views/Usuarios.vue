    ...
    <div class="layout-px-spacing">
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <button type="button" @click="openModal()" class="btn btn-info mb-2">Nuevo <i class="fas fa-user-lock"></i></button>
    ...
    methods: {
        ...
        backendResponse(response) {
            if(response.data.status == 'success'){
                ...
                if (this.rol_id == 0)
                    $('#select_rol').next().find('.select2-selection').addClass('has-error')
                alerts.sweetAlert(response.data.status, response.data.message)
            }
        },
        change_select_rol() {
